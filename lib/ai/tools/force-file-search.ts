 ...
 */
export function injectFileSearchDirective(
  ...
  const hasDirective = messages.some(
    (m) =>
      m.role === 'system' &&
      m.content?.toString().includes('IMMEDIATE ACTION REQUIRED')
  );
  ...
  const directive = createFileSearchDirective(
  ...
  console.log(
    '💉 Injected file search directive for:',
    `${lastUserMessage.content?.toString().substring(0, 50)}...`
  );
