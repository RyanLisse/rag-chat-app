...
export function enforceVectorSearch(messages: CoreMessage[]): CoreMessage[] {
  ...
  const hasRecentFileSearch = lastAssistantMessage?.content?.toString().includes('fileSearch') || 
  ...
  if (!hasRecentFileSearch) {
    ...
    console.log('🔍 Vector search enforcement activated for query:', latestUserMessage.content);
