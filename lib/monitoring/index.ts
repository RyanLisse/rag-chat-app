import { initializeTelemetry, shutdownTelemetry, ragMetrics, measureAsync, tracer } from './telemetry';
import { logger } from './logger';
import { initializeSentry, captureException, captureMessage, setUser } from './sentry';
import { context, trace, SpanStatusCode } from '@opentelemetry/api';

// Re-export commonly used functions
export { logger, captureException, captureMessage, setUser, ragMetrics, measureAsync };
...
export async function trackRAGOperation<T>(
  ...
  return measureAsync(`rag.${operation}`, async () => {
    const startTime = Date.now();
    ...
    try {
      ...
      switch (operation) {
        case 'vector_search':
          ...
          logger.logVectorSearch(metadata.query || '', metadata.resultCount || 0, duration);
          break;
          
        case 'model_inference':
          ragMetrics.modelInferenceDuration.record(duration, { model: metadata.model });
          if (metadata.tokens) {
            ragMetrics.modelTokensUsed.record(metadata.tokens, { model: metadata.model });
          }
        ...
        case 'document_processing':
          ...
          logger.logDocumentProcessing(metadata.documentId || '', metadata.chunkCount || 0, duration);
          break;
          
        default:
          logger.info(`RAG operation completed: ${operation}`, { ...metadata, duration });
      }
      ...
      logger.error(`RAG operation failed: ${operation}`, metadata, error as Error);
...
export function createRequestLogger(requestId: string, userId?: string, sessionId?: string) {
  return logger.child({
    requestId,
    userId,
    sessionId,
