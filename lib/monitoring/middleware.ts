...
export async function withMonitoring(
  ...
  try {
    // Log request start
    requestLogger.info(`Request started: ${request.method} ${request.nextUrl.pathname}`, {
      method: request.method,
    ...
    requestLogger.info(`Request completed: ${request.method} ${request.nextUrl.pathname}`, {
      method: request.method,
      path: request.nextUrl.pathname,
    ...
    // Log request error
    requestLogger.error(`Request failed: ${request.method} ${request.nextUrl.pathname}`, {
      method: request.method,
      path: request.nextUrl.pathname,
      duration,
    }, error as Error);
...
export function extractUserInfo(request: NextRequest): { userId?: string; sessionId?: string } {
...
export function trackChatMetrics(event: string, metadata?: Record<string, any>) {
  switch (event) {
    case 'message_sent':
...
export function createOperationSpan(name: string, attributes?: Record<string, any>) {
