...
export const logAggregationConfig: LogAggregationConfig = {
  ...
  batchSize: Number.parseInt(process.env.LOG_BATCH_SIZE || '100'),
  flushInterval: Number.parseInt(process.env.LOG_FLUSH_INTERVAL || '5000'), // 5 seconds
  maxRetries: Number.parseInt(process.env.LOG_MAX_RETRIES || '3'),
};
...
export class LogAggregator {
  ...
  private async sendToElasticsearch(logs: StructuredLog[]) {
    ...
    if (!response.ok) {
      throw new Error(
        `Elasticsearch bulk insert failed: ${response.statusText}`
      );
    }
  }
