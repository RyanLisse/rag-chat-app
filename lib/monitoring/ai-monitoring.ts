...
export function monitorStreamingResponse(
  ...
  return {
    ...
    onError: (error: Error) => {
      ...
      logger.error('Streaming model call failed', {
        ...metadata,
        duration,
        chunkCount,
        operation: 'model_inference',
...
export async function monitorVectorSearch<T>(
  ...
  return trackRAGOperation('vector_search', { query, ...metadata }, async () => {
    const result = await operation();
  ...
  return trackRAGOperation(
    ...
    async () => {
      ...
      if ('chunkCount' in result && typeof result.chunkCount === 'number') {
        ragMetrics.documentChunkCount.record(result.chunkCount, { documentType });
      }
