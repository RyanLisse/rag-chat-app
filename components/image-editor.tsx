...
export function ImageEditor({
  ...
  const imageSrc = isUrl || isDataUrl ? content : `data:image/png;base64,${content}`;
...
function parseMarkdownToHtml(markdown: string): string {
  ...
  // Convert images
  html = html.replace(/!\[([^\]]*)\]\(([^\)]+)\)/g, '<img src="$2" alt="$1" class="max-w-full h-auto rounded-lg shadow-lg my-4" />');
