...
import { CitationStatistics } from '@/lib/types/citation';
...
export function CitationStatisticsPanel({ statistics }: CitationStatisticsPanelProps) {
  const sourceTypeIcons = {
    document: <FileTextIcon className="w-4 h-4" />,
    webpage: <LinkIcon className="w-4 h-4" />,
    api: <ServerIcon className="w-4 h-4" />,
  ...
  const getQualityRating = (score: number) => {
    ...
    if (score >= 0.6) return { label: 'Good', color: 'bg-yellow-500' };
    if (score >= 0.4) return { label: 'Fair', color: 'bg-orange-500' };
    ...
    <div className="p-4 space-y-4">
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-lg font-semibold">{statistics.totalCitations}</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-lg font-semibold">{statistics.uniqueSources}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">Relevance Quality</span>
          </div>
          <Badge 
            variant="secondary" 
            className={`${qualityRating.color} text-white border-none`}
          >
            {qualityRating.label}
          </Badge>
        </div>
      ...
      <Card className="p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-medium">Source Types</span>
        </div>
        
        <div className="space-y-3">
          {Object.entries(statistics.sourceDistribution).map(([type, count]) => {
      ...
      <Card className="p-4">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <BarChart3Icon className="w-4 h-4 text-cyan-500" />
            <span className="text-sm font-medium">Citation Density</span>
          </div>
            ...
            <div>
              ...
              <p className="font-medium">
                {statistics.uniqueSources > 0 
                  ? (statistics.totalCitations / statistics.uniqueSources).toFixed(1)
                  : '0'
                }
              </p>
            </div>
      ...
      <Card className="p-4 bg-gray-50 dark:bg-gray-800/50">
        <h3 className="text-sm font-medium mb-2">Quality Insights</h3>
