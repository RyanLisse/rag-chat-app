...
export function CitationContent({
  ...
  const handleCitationClick = useCallback((citationId: string) => {
    ...
    if (citation) {
      const source = metadata.sources.find((s) => s.id === citation.source.id);
      if (source) {
  ...
  const handleCitationHover = useCallback((citationId: string, isHovering: boolean) => {
  ...
  const processedContent = useMemo(() => {
    ...
    metadata.citations.forEach((citation, index) => {
      ...
      result = result.replace(regex, `<citation-ref id="${citation.id}" number="${citationNumber}"></citation-ref>`);
    });
  ...
  const renderContentWithCitations = useCallback(() => {
    const parts = processedContent.split(/(<citation-ref[^>]*><\/citation-ref>)/);

    return parts.map((part, index) => {
      const match = part.match(/<citation-ref id="([^"]*)" number="([^"]*)"><\/citation-ref>/);
      ...
      if (match) {
        ...
        const citation = metadata.citations.find(c => c.id === citationId);
        const source = citation ? metadata.sources.find(s => s.id === citation.source.id) : null;
        ...
        return (
          <sup
            ...
            className={cn(
              ...
              isHighlighted && 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300',
              isSelected && 'bg-blue-500 text-white',
              !isHighlighted && !isSelected && 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'
            )}
  ...
  }, [processedContent, highlightedCitations, metadata.selectedCitationId, metadata.citations, metadata.sources, handleCitationClick, handleCitationHover]);
    ...
    <div className="flex flex-col lg:flex-row h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 sm:p-6 lg:p-8 xl:p-12 max-w-4xl mx-auto">
          <h1 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6" role="main" aria-label={`Citation article: ${title}`}>
            {title}
          </h1>
      ...
      <div className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-gray-800 flex flex-col bg-gray-50 dark:bg-gray-900/50 max-h-96 lg:max-h-none">
        <div className="p-4 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center justify-between">
            <h2 className="text-base lg:text-lg font-semibold" role="complementary" aria-label="Citations and sources sidebar">
              Sources & Citations
            </h2>
            <Button
              variant="ghost"
          ...
          <p className="text-sm text-muted-foreground mt-1" role="status">
            {metadata.citations.length} citations from {metadata.sources.length} sources
          </p>
        </div>
                    ...
                    return (
                      <Card
                        ...
                        className={cn(
                          ...
                          isRelatedHighlighted && 'ring-2 ring-blue-500 bg-blue-50 dark:bg-blue-900/20'
                        )}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between mt-2">
                              <Badge variant="secondary" className="text-xs">
                                {relatedCitations.length} citation{relatedCitations.length !== 1 ? 's' : ''}
                              </Badge>
        ...
        {metadata.citations.map((citation) => {
          ...
          return (
            <div key={citation.id} id={`citation-desc-${citation.id}`}>
              Citation {metadata.citations.indexOf(citation) + 1}: {citation.text}
              {source && ` Source: ${source.title}`}
              {citation.relevanceScore && ` Relevance: ${(citation.relevanceScore * 100).toFixed(0)}%`}
            </div>
          );
        })}
      </div>
      ...
      <div className="sr-only">
        {metadata.sources.map((source) => {
          const relatedCitations = metadata.citations.filter(c => c.source.id === source.id);
          return (
            <div key={source.id} id={`source-desc-${source.id}`}>
              ...
              {source.metadata?.date && ` Date: ${new Date(source.metadata.date).toLocaleDateString()}.`}
              {source.metadata?.excerpt && ` Excerpt: ${source.metadata.excerpt}`}
            </div>
          );
        })}
      </div>
      ...
      {selectedSource && (
        <SourcePreviewModal
          ...
          citations={metadata.citations.filter((c) => c.source.id === selectedSource.id)}
